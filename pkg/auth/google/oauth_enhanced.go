...
func writeJSONResponse(w http.ResponseWriter, data interface{}) {
	...
	// For now, we'll create the response manually to avoid import issues
	w.WriteHeader(http.StatusOK)
	
	// This is a simplified JSON response - in production you'd use json.Marshal
	switch response := data.(type) {
	case map[string]interface{}:
		fmt.Fprintf(w, `{"success": true, "data": %v}`, response)
	default:
		fmt.Fprintf(w, `{"success": true, "data": %v}`, response)
	}
}